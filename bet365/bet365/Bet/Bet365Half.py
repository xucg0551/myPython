            ...
            if md5 in self.collections.keys() and time_now != 0:
                ...
                if self.collections[md5]['start_time'] == None:
                    if handicap in userConfig.RULE_HALF['initial_handicaps'] and ratios != None:
                        ...
                        #确定是早晨还是晚上的比赛
                        morning_hour_range_min = userConfig.RULE_HALF['morning_hour_range']['min']
                        morning_hour_range_max = userConfig.RULE_HALF['morning_hour_range']['max']
                        self.collections[md5]['when'] = 'morning' if (datetime.now().hour >= morning_hour_range_min and datetime.now().hour <= morning_hour_range_max) else 'evening'
                ...
                when_key = self.collections[md5]['when']
                infos = userConfig.RULE_HALF['all_bets_info']['arleady_goals'][when_key]

                # 是否在规定的时间内满足进球数
                if all_goals not in infos:
                    if time_now > infos['cut_off_time']:  #截至时间
                        self.collections.pop(md5)
                    continue
                ...
                if (latest_goal_time >= infos[all_goals]['latest_goal_times']['min'] and latest_goal_time <= infos[all_goals]['latest_goal_times']['max']) == False:
                    self.collections.pop(md5)
                    continue
                ...
                if handicap_now_ok == False:
                    print('names={},all_goals->yes,latest_goal_time->yes,handicap={}->no'.format(names, handicap))
                    continue
                ...
                if times_betted == None:
                    print('names={},all_goals->yes,handicap->yes,latest_goal_time->yes,bet_times={}->no'.format(names, odds))
                    continue
